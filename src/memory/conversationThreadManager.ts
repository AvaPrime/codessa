...
export class ConversationThreadManager {
   ...
   */
  async createThread(title?: string, metadata?: Record<string, any>): Promise<string> {
  ...
  async addMessage(
    ...
    logger.debug(`Added message to thread ${threadId}: ${role} - ${content.substring(0, 100)}...`);
    return messageId;
  }
  ...
  async getThreadHistory(threadId: string, limit?: number): Promise<ConversationMessage[]> {
    ...
    logger.debug(`Retrieved ${messages.length} messages from thread ${threadId}`);
    return messages;
  }
  ...
  async getThread(threadId: string): Promise<ConversationThread | null> {
    const segment = this.memorySyncManager.getMemorySegment(`conversation_thread_${threadId}`);
   ...
   */
  async getRecentContext(threadId: string, limit: number = 10): Promise<string[]> {
  ...
  getMemoryStats(): any {
    ...
    const threadSegments = this.memorySyncManager.getSegmentsByType('episodic')
      .filter(segment => segment.id.startsWith('conversation_thread_'));
